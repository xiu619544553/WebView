...
@interface TKWebViewController () <WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate>
...
- (void)setupNavigationItem{
    ...
    [refreshButton setImage:[UIImage imageNamed:@"refresh_icon"] forState:UIControlStateNormal];
    ...
    UIBarButtonItem *ocInvokeJsItem = [[UIBarButtonItem alloc] initWithTitle:@"OC调用JS" style:UIBarButtonItemStyleDone target:self action:@selector(ocInvokeJsItemClick)];
    self.navigationItem.rightBarButtonItems = @[refreshButtonItem, ocInvokeJsItem];
...
- (void)ocInvokeJsItemClick {
    // changeColor()是JS方法名，completionHandler是异步回调block
    NSString *jsString = [NSString stringWithFormat:@"changeColor('%@')", @"oc调用js函数，并传递形参给js"];
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"local_pic" ofType:@"jpeg"];
...
// 被自定义的 WKScriptMessageHandler 在回调方法里通过代理回调回来，绕了一圈就是为了解决内存不释放的问题
...
- (void)userContentController:(WKUserContentController *)userContentController
    ...
    /*
     1、js方法名: message.name
         WKUserContentController 注册过的函数名，然后前端会在js中实现该方法，移动端就可以拦截；
     2、js传过来的参数: message.body
         注意：js方法传过来的参数是字符串，body就是字符串；
              传过来是字典，body就是字典；
         参数的数据类型必须约定好。
     */
    ...
    // js调用oc，拦截，处理业务
    if([methodName isEqualToString:@"jsToOcNoPrams"]) {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"js调用到了oc" message:@"不带参数" preferredStyle:UIAlertControllerStyleAlert];
        [alertController addAction:([UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        }])];
        [self presentViewController:alertController animated:YES completion:nil];
    } else if([methodName isEqualToString:@"jsToOcWithPrams"]){
...
- (WKWebView *)webView {
    if(!_webView){
        ...
        TKWeakWebViewScriptMessageDelegate *scriptMessageDelegate = [[TKWeakWebViewScriptMessageDelegate alloc] initWithDelegate:self];
        
        
        // 这个类主要用来做native与JavaScript的交互管理
        WKUserContentController *userContentController = [[WKUserContentController alloc] init];
        // 注册一个name为jsToOcNoPrams的js方法 设置处理接收js方法的对象，注意：需要在 -dealloc 方法中 remove
        [userContentController addScriptMessageHandler:scriptMessageDelegate
                                                  name:@"jsToOcNoPrams"];
        [userContentController addScriptMessageHandler:scriptMessageDelegate
...
//        WKBackForwardList *backForwardList = [_webView backForwardList];
        ...
        [_webView evaluateJavaScript:@"navigator.userAgent" completionHandler:^(id result, NSError *error) {
            ...
            NSString *newAgent = @"app/ios/1.00.00";
...
- (void)loadNetworkHtml {
    NSString *htmlUrl;
    htmlUrl = @"http://qtwl-h5.3kedu.com/nonmember/";
    
    // 在 WKWebView 中加载下面的测试链接可以稳定重现白屏现象
//    htmlUrl = @"http://people.mozilla.org/~rnewman/fennec/mem.html";

    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:htmlUrl]];
    [request addValue:[self readCurrentCookieWithDomain:htmlUrl] forHTTPHeaderField:@"Cookie"];
